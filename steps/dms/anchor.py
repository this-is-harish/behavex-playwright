...
def execute_command(context, key_condition: str):
    context.keys = context.keys+"random" if key_condition.startswith("invalid") else context.keys
    command = f"{context.nunet_path} cap anchor --context dms --root {context.keys}"
    context.anchor_output = terminal_helper.run_command_with_passphrase(command=command,passphrase=context.passphrase)

@step('specified user {is_or_is_not} added root anchor')
def verify_anchoring(context, is_or_is_not):
    if is_or_is_not == "is not":
        assert context.anchor_output.wait() !=0
    else:
        assert context.anchor_output.wait() == 0
...
def grant_user_context(context, condition: str):
    ...
    context.grant_output = terminal_helper.run_command_with_passphrase(command=command, passphrase=context.passphrase)

@step('user {is_or_is_not} granted access successfully')
def grant_access_success(context, is_or_is_not):
    if is_or_is_not == "is not":
        assert str(context.grant_output).split("\n")[-1].strip().__contains__("Error: invalid argument")
    else:
        assert context.grant_output.wait() == 0
