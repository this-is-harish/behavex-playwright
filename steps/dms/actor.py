...
@step('user executes command to create new {user_condition} with "{passphrase_condition}" passphrase')
def create_new_user(context,user_condition:str, passphrase_condition: str):
    ...
    context.user_name = names.get_first_name().lower()
    ...
    terminal_helper.run_command_with_passphrase(command=command, passphrase=context.passphrase, confirm_passphrase=True, incorrect_passphrase=incorrect_passphrase)
